...
![.github/workflows/ci.yml](https://github.com/rickshapirobetter/terraform-provider-snowflake/workflows/.github/workflows/ci.yml/badge.svg)

This is a terraform provider plugin for managing [Snowflake](https://www.snowflake.com/) accounts.

## Getting Help

If you need help, try the [discussions area](https://github.com/rickshapirobetter/terraform-provider-snowflake/discussions) of this repo.
...
You can also just download a binary from our [releases](https://github.com/rickshapirobetter/terraform-provider-snowflake/releases) and follow the [Terraform directions for installing 3rd party plugins](https://www.terraform.io/docs/configuration/providers.html#third-party-plugins).

### For Terraform v0.13+ users

> We are now (7/29/2021) using Terraform 0.13 for testing purposes due to an issue for data sources for versions <0.13. Related PR for this change [here](https://github.com/rickshapirobetter/terraform-provider-snowflake/pull/622#issuecomment-888879621).
...
The 'internal' tests are run in the `github.com/rickshapirobetter/terraform-provider-snowflake/pkg/resources` package so that they can test functions that are not exported. These tests are intended to be limited to unit tests for simple functions.

The 'unit' tests are run in  `github.com/rickshapirobetter/terraform-provider-snowflake/pkg/resources_test`, so they only have access to the exported methods of `resources`. These tests exercise the CRUD methods that on the terraform resources. Note that all tests here make use of database mocking and are run locally. This means the tests are fast, but are liable to be wrong in subtle ways (since the mocks are unlikely to be perfect).
